"""One deduped buyer registry for the 6-county DC/MD/VA footprint, ready to skip trace and text.

(Ported 2026-09-07 from Ty's Knox/Blount original, upstream b5de920: only the
COUNTIES constant, the TN state fallbacks and the report caveat changed.)
...
these counties, and what is their phone number".
...
# The 6-county footprint (fips is the load-bearing field; per the sweep notes
# above, a wrong name segment is indistinguishable from an empty county, so
# check the per-county counts on a first run). Ty's original ran Knox/Blount TN.
COUNTIES = [
...
STATE_FULL = {"TN": "TENNESSEE", "MD": "MARYLAND", "VA": "VIRGINIA",
...
def phase_sweep(args) -> None:
    ...
    gate(total_rows > 0, "sweep returned zero rows across all counties")
...
def _officer_in_state(officer: dict, state: str) -> bool:
    ...
    st = (state or "").upper()
    if not st:
        # No state to verify against means unverifiable, and the comment above
        # says geography filtering is mandatory, not a refinement.
        return False
    tokens = "".join(c if c.isalnum() else " " for c in addr).split()
    if st in tokens:
        return True
    full = STATE_FULL.get(st)
    return bool(full) and full in " ".join(tokens)
...
def phase_principals(args) -> None:
    ...
    for i, key in enumerate(todo, 1):
        ...
        # Fallback state comes from where the buyer actually bought, not a
        # hardcoded market (Ty's original defaulted to TN here).
        fallback_st = next((st for _, cname, st in COUNTIES
                            if cname in (b.get("counties") or [])), "")
        state = (b.get("mail", "").strip()[-8:].split()[0] if b.get("mail") else "") or fallback_st
...
def write_profiles_xlsx(profiles: list, path: Path) -> None:
    ...
    srows = [
        ["Source", "SiftMap /properties/search/ and /properties/detail/, "
                   + ", ".join(f"{n} ({f})" for f, n, _ in COUNTIES)
                   + ", single family only"],
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Cash-buyer registry for the 6-county DC/MD/VA footprint")
